...
> **Full content**: [docs/implementation.md](docs/implementation.md) — CLI, UI layout, HTML structure, CSS rules, canvas rendering details.
...
#### 2.13.4 Verification Contract

Machine-parseable acceptance criteria per §2.3 feature. DO/CHECK subagents consume this block via `~/.claude/scripts/extract_contract.sh`. Edit the `acceptance:` lists to concrete testable assertions.

<!-- contract-block-start -->
```yaml
- feature_id: "2.3 #1"
  description: "textbox — `TextBoxComponent` | Free text (multi-line via `\n`) | 10×1 | 1×1 | `text: 'Text'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #2"
  description: "box — `BoxComponent` | `┌─┐│ │└─┘` rectangle | 12×6 | 3×3 | —"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #3"
  description: "button — `ButtonComponent` | `[ Label ]` | 10×1 | 5×1 | `label: 'Button'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #4"
  description: "input — `InputComponent` | `[___________]` or `[placeholder]` | 16×1 | 5×1 | `placeholder: ''`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #5"
  description: "card — `CardComponent` | Box + title row + `├──┤` divider at row 2 | 20×8 | 5×4 | `title: 'Card Title'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #6"
  description: "table — `TableComponent` | Outer box + `┬┼┴` vertical dividers + header divider | 24×7 | 5×4 | `cols:3, rows:3, headers:['Col1','Col2','Col3']`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #7"
  description: "modal — `ModalComponent` | Box + title + `×` close + header divider + action row `[ Cancel ] [ OK ]` | 30×12 | 10×6 | `title: 'Modal Title'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #8"
  description: "tabs — `TabsComponent` | `[Active] Tab2 Tab3` + `───` underline | 30×2 | 8×2 | `tabs:['Tab 1','Tab 2','Tab 3'], activeIndex:0`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #9"
  description: "navbar — `NavBarComponent` | `Logo  Home  About  Contact  [Sign In]` | 40×1 | 10×1 | `logo:'Logo', links:['Home','About','Contact'], action:'Sign In'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #10"
  description: "dropdown — `DropdownComponent` | `[Option   ▾]` | 14×1 | 5×1 | `value: 'Option'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #11"
  description: "search — `SearchComponent` | `[/ Search...     ]` | 20×1 | 8×1 | `placeholder: 'Search...'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #12"
  description: "checkbox — `CheckboxComponent` | `[x] Label` / `[ ] Label` | 12×1 | 4×1 | `label:'Option', checked:true`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #13"
  description: "radio — `RadioComponent` | `(o) Label` / `( ) Label` | 12×1 | 4×1 | `label:'Option', selected:true`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #14"
  description: "toggle — `ToggleComponent` | `[━●] On` / `[●━] Off` | 8×1 | 5×1 | `on:true, label:''`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #15"
  description: "progress — `ProgressComponent` | `[████░░░░] 50%` | 16×1 | 8×1 | `value: 50`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #16"
  description: "breadcrumb — `BreadcrumbComponent` | `Home > Docs > About` | 24×1 | 5×1 | `items:['Home','Docs','About']`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #17"
  description: "pagination — `PaginationComponent` | `< 1 [2] 3 4 5 >` | 16×1 | 7×1 | `pages:5, current:1`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #18"
  description: "separator — `SeparatorComponent` | `────────────────────` | 20×1 | 2×1 | —"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #19"
  description: "line — `LineComponent` | `────────` or `│` (vertical) | 8×1 | 1×1 | `orientation:'horizontal'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
- feature_id: "2.3 #20"
  description: "arrow — `ArrowComponent` | `───────→` or `↓` | 8×1 | 2×1 | `orientation:'horizontal', direction:'right'`"
  acceptance:
    - "TODO: replace with a testable assertion (see §2.13.1)"
```
<!-- contract-block-end -->
